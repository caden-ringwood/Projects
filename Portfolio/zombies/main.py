...
class Game:
    ...
    def load_data(self):
        ...
        for snd in PLAYER_HIT_SOUNDS:
            self.player_hit_sounds.append(pg.mixer.Sound(path.join(sounds_folder, snd)))
        self.zombie_hit_sounds = []
        for snd in ZOMBIE_HIT_SOUND:
            self.zombie_hit_sounds.append(pg.mixer.Sound(path.join(sounds_folder, snd)))
    ...
    def new(self):
        ...
        self.map = Tiled_Map(path.join(self.map_folder, "map_1.tmx"))
        ...
        for tile_obj in self.map.tmxdata.objects:
            ...
            if tile_obj.name in ["health", "shotgun"]:
    ...
    def render_fog(self):
        # draw light mass onto fog image
        self.fog.fill(NIGHT_COLOR)
